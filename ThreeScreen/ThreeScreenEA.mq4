...
extern int MagicNumber = 20170824;                       //Magic
...
//--- Money Management
    ...
    int lowerMATrend = 0;
    double lowerMaHigh = iMA( NULL, Low_TF, 5, 2, MODE_SMMA, PRICE_HIGH, 1 );
    double lowerMaLow = iMA( NULL, Low_TF, 5, 2, MODE_SMMA, PRICE_LOW, 1 );
    if(iClose( NULL, Low_TF, 1 ) > lowerMaLow) lowerMATrend = 1;
    if(iClose( NULL, Low_TF, 1 ) < lowerMaHigh) lowerMATrend = -1;

    int wprCross = 0;
    double wprValue = iWPR(NULL, Low_TF , 14 , 1);
    double wprPreValue = iWPR(NULL, Low_TF , 14 , 2);
    ...
    int macdCross = 0;
    double MacdMain = iMACD( NULL, Low_TF,  fast_ema_period,  slow_ema_period,  signal_period,  PRICE_CLOSE, MODE_MAIN, 1 );
    double MacdMainPre = iMACD( NULL, Low_TF,  fast_ema_period,  slow_ema_period,  signal_period,  PRICE_CLOSE, MODE_MAIN, 2 );
    double MacdSignal = iMACD( NULL, Low_TF,  fast_ema_period,  slow_ema_period,  signal_period,  PRICE_CLOSE, MODE_SIGNAL, 1 );
    double MacdSignalPre = iMACD( NULL, Low_TF,  fast_ema_period,  slow_ema_period,  signal_period,  PRICE_CLOSE, MODE_SIGNAL, 2 );
    ...
    int cciCross = 0;
    double FXSCurrent = iCustom(Symbol(),Low_TF,"FX_Snipervs_T3_CCI",CCI_Period,T3_Period,b,0,1);
    double FXSLast1   = iCustom(Symbol(),Low_TF,"FX_Snipervs_T3_CCI",CCI_Period,T3_Period,b,0,2);
    double FXSLast2   = iCustom(Symbol(),Low_TF,"FX_Snipervs_T3_CCI",CCI_Period,T3_Period,b,0,3);
  ...
  bool is_trend = false, cross = false;
   ...
   if ( ichomuTrend==1 && direction == 1 && lowerMATrend == 1) {
