...
/*
https://www.forexfactory.com/showthread.php?t=91009
...
extern int MagicNumber = 20170927;                       //Magic
...
int init() {
  if(Period() != 60){
    Alert("This system only work for H1 timeframe!");
  }
...
int start(){
  ...
  if(TotalOrdersCount() < MaxTrade) {
    if(preTime < Time[0]){
      // Open bar should be the first 3 bars to cross over the ma channel
      // Open bar should be a red bar for sell and green bar for buy
    ...
    // Use opposite signal as exit signal
    int exit_signal = getSignal();
    // Use ma cross over as exit signal
    exit_signal = getExitSignal();
...
int getSignal(){
  ...
  double haClose = iClose( NULL, Low_TF, 1);

  // if(haClose > maHigh && (getindexofbreak(1) == 1 || getindexofbreak(1) == 2)) maChannelCross = 1;
  // if(haClose < maLow && (getindexofbreak(-1) == 1 || getindexofbreak(-1) == 2)) maChannelCross = -1;
  if(haClose > maHigh) maChannelCross = 1;
  if(haClose < maLow) maChannelCross = -1;

  int rsichart = 0;
  double rsi55 = iRSI( NULL, Low_TF, 55, PRICE_CLOSE, 1);
  if(rsi55 > 50) rsichart = 1;
  if(rsi55 < 50) rsichart = -1;

  int rsioma_cross = 0;
  int index = -1,direction = 0;
  double rsioma_signal,risoma_main;
  for(int i = 1; i < 20; i++){
    rsioma_signal = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 5,i);
    risoma_main = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 0,i);
    // initialize direction
    if(direction == 0){
      if(rsioma_signal > risoma_main) direction = 1;
      else if(rsioma_signal < risoma_main) direction = -1;
    }
    if(direction == 1){
      if(rsioma_signal < risoma_main) {
        index = i;
        break;
      }
    } else if(direction == -1){
  ...
  rsioma_signal = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 0,0);
  risoma_main = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 5,0);
  double rsioma_signal1 = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 0,1);
  double risoma_main1 = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 5,1);
  if(rsioma_signal > risoma_main && rsioma_signal1 < risoma_main1) rsioma_cross=1;
  if(rsioma_signal < risoma_main && rsioma_signal1 > risoma_main1) rsioma_cross=-1;
  
  if(rsioma_signal > risoma_main) rsioma_cross=1;
  if(rsioma_signal < risoma_main) rsioma_cross=-1;
  //if(index < 3 && index > -1) rsioma_cross = direction;
  
  int wpr_cross = 0;
  double wpr0 = iWPR(NULL,Low_TF,55,0);
  double wpr1 = iWPR(NULL,Low_TF,55,1);
  if(wpr0 > -20 && wpr1 < -20) wpr_cross = 1;
  if(wpr0 < -80 && wpr1 > -80) wpr_cross = -1;
  ...
  if ( rsichart == 1 && maChannelCross == 1 && rsioma_cross == 1 && wpr_cross == 1) signal_1 = 1;
  if ( rsichart == -1 && maChannelCross == -1 && rsioma_cross == -1  && wpr_cross == -1) signal_1 = -1;
   //if(rsioma_cross == 1 && risoma_main < 80) signal_1 = 1;
   //if(rsioma_cross == -1 && risoma_main > 20) signal_1 = -1;
  signal_2 = signal_1;
...
int getExitSignal(){
  int maChannelCross = 0;
  double rsioma_signal = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 0,0);
  double risoma_main = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 5,0);
  double rsioma_signal1 = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 0,1);
  double risoma_main1 = iCustom( NULL, Low_TF, "RSIOMA (2)",14,1,0,21,1, 5,1);
  if(rsioma_signal > risoma_main && rsioma_signal1 < risoma_main1) maChannelCross=1;
  if(rsioma_signal < risoma_main && rsioma_signal1 > risoma_main1) maChannelCross=-1;
  
  return maChannelCross;
}

int getindexofbreak(int direction){
  double haClose,maHigh,maLow;
  int index = -1;
  for(int i = 1; i < 20; i++){
    haClose = iCustom(NULL, Low_TF, "Heiken Ashi", 0,0,0,0, 3, i);
    maHigh = iMA( NULL, Low_TF, MaPeriod, MaShift, MaMode, PRICE_HIGH, i);
    maLow = iMA( NULL, Low_TF, MaPeriod, MaShift, MaMode, PRICE_LOW, i);
    if(direction == 1){
      if(haClose < maHigh) {
        index = i;
        break;
